...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const renderCalendar = () => {
        ...
        for (let d = 1; d <= daysInMonth; d++) {
            ...
            div.textContent = d;
            div.className = `cursor-pointer p-1 hover:bg-gray-200 rounded ${isToday ? "today" : ""} ${isSelected ? "selected-date" : ""}`;
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (openBtn) openBtn.onclick = () => document.getElementById("timeSlotForm").classList.remove("hidden");
    if (closeBtn) closeBtn.onclick = () => document.getElementById("timeSlotForm").classList.add("hidden");
    if (calendarToggleBtn) calendarToggleBtn.onclick = () => calendarWrapper.classList.toggle("hidden");
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const renderCalendar = () => {
        ...
        for (let d = 1; d <= daysInMonth; d++) {
            ...
            const div = document.createElement("div");
            div.textContent = d;
            div.className = `cursor-pointer p-1 hover:bg-gray-200 rounded text-sm ${isToday ? "bg-blue-100" : ""}`;
            div.addEventListener("click", () => {
                selectedDate = date;
                calendar.classList.add("hidden");
                renderCalendar();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function resetStars() {
        const activeRating = document.querySelector('.rating-stars').getAttribute('data-active-rating') || 0;
        highlightStars(activeRating);
    }
    function setRating(rating) {
        document.querySelector('.rating-stars').setAttribute('data-active-rating', rating);
        highlightStars(rating);
    }
});
